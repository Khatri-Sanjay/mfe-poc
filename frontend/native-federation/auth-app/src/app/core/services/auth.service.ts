...
export class AuthService {
  ...
  readonly isAdmin = computed(() => {
    const u = this._user();
    if (!u) return false;
    return u.roles.some((r) => r.toUpperCase() === 'ADMIN' || r.toUpperCase() === 'SUPER_ADMIN');
  });
  ...
  resendVerification(data: ResendVerificationRequest): Observable<MessageResponse> {
    ...
    return this.http.post<MessageResponse>(`${this.apiUrl}/auth/resend-verification`, data).pipe(
      tap(() => this._isLoading.set(false)),
      catchError((error) => {
        this._isLoading.set(false);
        return this.handleError(error);
  ...
  hasRole(user: User, role: string): boolean {
    ...
    return user.roles.some((userRole) => {
      ...
      return (
        normalizedUserRole === normalizedRole || normalizedUserRole === `ROLE_${normalizedRole}`
      );
    });
  }
