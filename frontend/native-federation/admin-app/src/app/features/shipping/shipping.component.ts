    ...
    <div class="admin-page-content">
      @if (showForm()) {
        <div class="form-section">
          <div class="resource-form">
            ...
            /></label>
            <label
              >Code <input type="text" [(ngModel)]="editForm.code" required placeholder="STANDARD"
            /></label>
            <label
              >Price <input type="text" [(ngModel)]="editForm.price" required placeholder="10.00"
            /></label>
            <label
              >Currency <input type="text" [(ngModel)]="editForm.currency" placeholder="AUD"
            /></label>
            <label
              >Min Days <input type="number" [(ngModel)]="editForm.estimatedMinDays" min="0"
            /></label>
            <label
              >Max Days <input type="number" [(ngModel)]="editForm.estimatedMaxDays" min="0"
            /></label>
            <label class="span-2"
              >Description
              <textarea
      ...
      @if (loading()) {
        ...
        <div class="empty-state">
          ...
          <h2>No shipping methods</h2>
        </div>
      } @else {
        <div class="data-table">
          <div class="data-table-header">
            <div class="data-row cols-6 header">
              <span>Name</span><span>Code</span><span>Price</span><span>Est. Days</span
          ...
          <div class="data-table-body">
            @for (m of methods(); track m.id) {
              <div class="data-row cols-6">
                ...
                <span
                  ><span class="status" [class.stock]="m.isActive" [class.in]="m.isActive">{{
                    m.isActive ? 'Active' : 'Inactive'
                  }}</span></span
                >
                <span class="table-actions">
                  <button class="icon-btn compact" (click)="edit(m)">
                    <i class="bi bi-pencil"></i>
                  </button>
                  <button class="icon-btn danger compact" (click)="delete(m)">
...
export class ShippingComponent implements OnInit {
  ...
  save() {
    ...
    const req = id
      ? this.http.patch(`${environment.apiUrl}/admin/shipping/methods/${id}`, body)
      : this.http.post(`${environment.apiUrl}/admin/shipping/methods`, body);
    req.subscribe({
      next: () => {
        this.toast.success('Saved', 'Shipping method saved.');
        this.cancelEdit();
        this.load();
      },
  ...
  delete(m: ShippingMethod) {
    ...
    this.http.delete(`${environment.apiUrl}/admin/shipping/methods/${m.id}`).subscribe({
      next: () => {
        this.toast.success('Deleted', 'Shipping method deleted.');
        this.load();
      },
      error: (err) => this.toast.error('Error', err.error?.message || 'Failed.'),
    });
