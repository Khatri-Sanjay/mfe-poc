...
import type { ApiResponse, Product, ProductQuery } from '../types/product.types';
import { productCategoryLabel, productIsActive, productPrice, productRelationLabel, productStock } from '../utils/product-display';
import { queryFromSearchParams, queryToSearchParams } from '../utils/product-query';
...
export function ProductListScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setResponse(await productsApi.list(query));
    } catch (err) {
  ...
  async function handleDelete(product: Product) {
    ...
    try {
      ...
      window.alert(getApiErrorMessage(err, 'Unable to delete product.'));
    }
  }
    ...
    <main className="page stack">
      <header className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage products, images, variants, pricing and stock.</p>
        </div>
      ...
      <form className="card filters" onSubmit={applyFilters} key={searchParams.toString()}>
        <div className="grid grid-4">
          ...
          <Field label="Stock">
            <select className="select" name="inStock" defaultValue={query.inStock === undefined ? '' : String(query.inStock)}>
          ...
          <Field label="Sort by">
            <select className="select" name="sortBy" defaultValue={query.sortBy ?? 'createdAt'}>
              <option value="createdAt">Created</option><option value="name">Name</option><option value="price">Price</option>
            </select>
          </Field>
          <Field label="Sort order">
            <select className="select" name="sortOrder" defaultValue={query.sortOrder ?? 'desc'}>
              <option value="desc">Descending</option><option value="asc">Ascending</option>
            </select>
          </Field>
        </div>
        <div className="filter-actions">
          <button className="btn" type="button" onClick={() => router.push(pathname)}>Clear</button>
      ...
      <section className="card">
        {loading ? <LoadingBlock label="Loading products..." /> : products.length === 0 ? (
          <div className="empty">No products matched the current filters.</div>
        ) : (
                    ...
                    <tr key={product.id}>
                      <td>
                        <div className="product-cell">
                          {image?.url ? <img className="thumb" src={image.url} alt={image.altText ?? product.name} /> : <div className="thumb" />}
                          <div><div className="product-name">{product.name}</div><div className="product-slug">{product.slug}</div></div>
                        </div>
                      </td>
                      <td data-label="Brand">{productRelationLabel(product.brand)}</td>
                      <td data-label="Category">{productCategoryLabel(product)}</td>
                      <td data-label="Price">{money(productPrice(product))}</td>
                      <td data-label="Stock">{productStock(product)}</td>
                      ...
                      <td data-label="Actions"><div className="actions"><Link className="btn btn-small" href={`/admin/products/${product.id}/edit`}>Edit</Link><button className="btn btn-small btn-danger" onClick={() => void handleDelete(product)}>Delete</button></div></td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {meta && meta.totalPages > 1 && (
          <div className="pagination">
            <div className="pagination-info">Page {meta.page} of {meta.totalPages} · {meta.total} total</div>
            <div className="row">
              <button className="btn btn-small" disabled={meta.page <= 1} onClick={() => replaceQuery({ ...query, page: meta.page - 1 })}>Previous</button>
              <button className="btn btn-small" disabled={meta.page >= meta.totalPages} onClick={() => replaceQuery({ ...query, page: meta.page + 1 })}>Next</button>
            </div>
          </div>
        )}
      </section>
