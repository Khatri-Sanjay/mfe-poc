...
@Component({
  standalone: true,
  imports: [ReactiveFormsModule, RouterLink, NgbPaginationModule, MoneyPipe, EmptyStateComponent],
  template: `
                ...
                <div class="product-content">
                  ...
                  <div class="product-row">
                    ...
                    <div class="inline-actions">
                      ...
                      <button class="btn-primary compact" type="button" [disabled]="stock(product) === 0" (click)="cart.addProduct(product)">
...
export class ProductListPage implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.filters.patchValue({
      ...
      category: this.route.snapshot.queryParamMap.get('category') ?? (parentPath.startsWith('categories') ? parentSlug : ''),
      brand: this.route.snapshot.queryParamMap.get('brand') ?? (parentPath.startsWith('brands') ? parentSlug : ''),
  ...
  async addWishlist(product: Product): Promise<void> {
    if (!this.auth.isAuthenticated()) {
      this.notifications.info('Sign in to save products.');
